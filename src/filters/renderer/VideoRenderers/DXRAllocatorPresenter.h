...
namespace DSObjects
{
        ...
        : public CSubPicAllocatorPresenterImpl
    {
            ...
            : public ISubRenderCallback2
        {
            ...
            CDXRAllocatorPresenter* m_pDXRAP;// doesn't hold a reference
            CCritSec m_csLock;
            ULONG volatile mv_ulReferenceCount;

        public:
            __declspec(nothrow noalias) __forceinline CSubRenderCallback(CDXRAllocatorPresenter* pDXRAP)
                : m_pDXRAP(pDXRAP)
...
exit:
            ...
            __declspec(nothrow noalias) void __forceinline SetDXRAP(CDXRAllocatorPresenter* pDXRAP) {
        ...
        __declspec(nothrow noalias) __forceinline ~CDXRAllocatorPresenter() {
            if (m_pSRCB) {
    ...
    public:
        ...
        __declspec(nothrow noalias) __forceinline CDXRAllocatorPresenter(__in HWND hWnd, __inout CStringW* pstrError, __out_opt IBaseFilter** ppRenderer)
            ...
            HRESULT hr;
            __assume(this);// fix assembly: the compiler generated tests for null pointer input on static_cast<T>(this)
            if (FAILED(hr = CoCreateInstance(CLSID_DXR, static_cast<IUnknown*>(static_cast<CSubPicAllocatorPresenterImpl*>(this)), CLSCTX_ALL, IID_IUnknown, reinterpret_cast<void**>(&m_pDXR)))) {// CSubPicAllocatorPresenterImpl is at Vtable location 0
                ASSERT(0);
                *pstrError = L"CoCreateInstance() of Haali Renderer failed\n";
            ...
            if (FAILED(hr = m_pDXR->QueryInterface(__uuidof(ISubRender), reinterpret_cast<void**>(&pSR)))) {
                ...
                m_pDXR->Release();
                m_pDXR = nullptr;
                *pstrError = L"failed interface query for ISubRender to Haali Renderer\n";
                *pstrError += GetWindowsErrorMessage(hr, nullptr);
                return;
            }
            ...
            if (!pRawMem) {
                ...
                m_pDXR->Release();
                m_pDXR = nullptr;
                *pstrError = L"Out of memory error for creating CSubRenderCallback for Haali Renderer\n";
                return;
            }
            ...
            if (FAILED(hr)) {
                ...
                m_pDXR->Release();
                m_pDXR = nullptr;
                *pstrError = L"failed to set ISubRenderCallback on Haali Renderer\n";
                *pstrError += GetWindowsErrorMessage(hr, nullptr);
                return;
            }
            ...
            if (FAILED(hr = m_pDXR->QueryInterface(IID_IBaseFilter, reinterpret_cast<void**>(ppRenderer)))) {
                ...
                m_pDXR->Release();
                m_pDXR = nullptr;
                *pstrError = L"failed interface query for IBaseFilter to Haali Renderer\n";
                *pstrError += GetWindowsErrorMessage(hr, nullptr);
            }
        }
    };
