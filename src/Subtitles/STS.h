...
public:
    ...
    bool const mk_bRenderedTextType;
...
public:
    CSimpleTextSubtitle(bool bRenderedTextType = false);
    ...
    LONGLONG TranslateStart(__in const size_t i, __in const double fps) const;
    LONGLONG TranslateEnd(__in const size_t i, __in const double fps) const;
    size_t SearchSub(__int64 t, double fps);

    LONGLONG TranslateSegmentStart(__in const size_t i, __in const double fps) const;
    LONGLONG TranslateSegmentEnd(__in const size_t i, __in const double fps) const;
    STSSegment const* SearchSubs(LONGLONG t, double fps, size_t* iSegment = NULL, size_t* nSegments = NULL) const;
    __declspec(nothrow noalias restrict) STSSegment const* GetSegment(__in size_t iSegment) const {
        return (iSegment < m_segments.GetCount()) ? &m_segments[iSegment] : NULL;
    }

    STSStyle* GetStyle(size_t i);
    bool GetStyle(size_t i, STSStyle& stss);
