...
void CAppSettings::UpdateData(bool fSave)
{
	...
	if (fSave) {
		...
		iDSVideoRendererType = pApp->GetProfileInt(IDS_R_SETTINGS, IDS_RS_DSVIDEORENDERERTYPE, (IsWinVistaOrLater() ? (((GetRenderersData()->GetD3X9Dll() != NULL) && CMPlayerCApp::HasEVR()) ? VIDRNDT_DS_EVR_CUSTOM : VIDRNDT_DS_DEFAULT) : VIDRNDT_DS_OVERLAYMIXER) );
		...
		// Internal filters
		for (int f=0; f<SRC_LAST; f++) {
			// In Soviet CCCP we wish to disable certain internal filters.
			switch (f) {
				case SRC_AVI:
				case SRC_MATROSKA:
				case SRC_MP4:
		...
		for (int f=0; f<TRA_LAST; f++) {
			switch (f) {
				...
				case TRA_AMR:
				case TRA_MPEG1:
					SrcFilters[f] = !!pApp->GetProfileInt(IDS_R_INTERNAL_FILTERS, TraFiltersKeys[f], 0);
					break;
				default:
